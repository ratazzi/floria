...
final class CloudRecordSyncSession: NSObject, CKSyncEngineDelegate, @unchecked Sendable {
    ...
    private func handleSentRecords(
        ...
        let resolution = try CloudSentBatchResolver(codec: codec).resolve(
            phase: phase,
...
private actor CloudRecordSyncSessionState {
...
enum CloudRecordSyncSessionError: Error, Equatable, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .accountChanged(let message), .cloudKit(let message), .damaged(let message),
