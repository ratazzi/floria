...
final class ProductLanguageTests: XCTestCase {
    ...
    func testProjectSshAccessHidesFloriaRuntimeSocketDetails() throws {
        ...
        XCTAssertTrue(workspace.contains(#"Text("SSH identities")"#))
        XCTAssertTrue(workspace.contains(#".popover(isPresented: $showingIdentityPicker"#))
        XCTAssertTrue(workspace.contains(#".toggleStyle(.checkbox)"#))
        XCTAssertTrue(workspace.contains("let sourceSummary = identitySourceSummary(for: provider)"))
        XCTAssertTrue(workspace.contains("Text(identity.sourceSummary)"))
        XCTAssertTrue(workspace.contains("sshFingerprint(from: address)"))
        XCTAssertFalse(workspace.contains(#"Text("Identity source")"#))
        XCTAssertFalse(workspace.contains(#"GroupBox("Allowed identities")"#))
