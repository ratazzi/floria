...
fn run_script(argv: &[String], ctx: &SourceCtx<'_>) -> Result<Vec<u8>> {
    ...
    let mut child = Command::new(program)
        ...
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .env("FLORIA_OPERATION", ctx.operation.as_str())
        .env("FLORIA_PATH", ctx.virtual_path)
        .env("FLORIA_REQUEST_UID", ctx.request_uid.to_string())
        .env("FLORIA_REQUEST_PID", ctx.request_pid.to_string())
        .spawn()
...
mod tests {
    ...
    fn command_source_captures_stdout_and_request_context() {
        ...
        let source = CommandSource::new(vec![
            "/bin/sh".to_string(),
            "-c".to_string(),
            "printf 'op=%s p=%s u=%s path=%s' \"$FLORIA_OPERATION\" \
                 \"$FLORIA_REQUEST_PID\" \"$FLORIA_REQUEST_UID\" \"$FLORIA_PATH\""
                .to_string(),
        ]);
