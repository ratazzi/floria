...
#[derive(Debug, Deserialize)]
pub struct StoreCfg {
    /// Root directory for encrypted secret blobs. Defaults to `~/Library/Application Support/floria/store`.
...
impl Config {
    ...
    pub fn resolve(self, base_dir: &Path) -> Result<ResolvedConfig> {
        ...
        let volname = self.mount.volname.unwrap_or_else(|| "Floria".to_string());
...
/// Default secret store root: `~/Library/Application Support/floria/store`.
fn default_store_root() -> PathBuf {
    let base = std::env::var_os("HOME")
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("."));
    base.join("Library/Application Support/floria/store")
}
...
fn default_audit_path(mount_path: &Path) -> PathBuf {
    ...
    let name = mount_path
        ...
        .unwrap_or_else(|| "floria.audit.jsonl".to_string());
    mount_path
        .parent()
