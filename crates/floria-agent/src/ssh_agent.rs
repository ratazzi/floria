...
impl RunningSurface {
    fn start(
        ...
        let thread = std::thread::Builder::new()
            .name(format!("floria-ssh-agent-{}", spec.id))
            .spawn(move || {
                accept_loop(
                    listener,
                    loop_spec,
...
fn peer_identity(stream: &UnixStream) -> Option<ProcessIdentity> {
    ...
    match peer_pid(stream) {
        Some(pid) => Some(floria_platform::enrich(pid, uid, gid)),
...
impl ProviderSession {
    ...
    fn identities(&mut self) -> io::Result<HashMap<String, ParsedIdentity>> {
        match &self.spec {
            ProviderSpec::ManagedPrivateKey { secret_id, .. } => {
                ...
                let identity = floria_ssh::identity_from_private_key(&encoded)
                    .map_err(|error| invalid(error.to_string()))?;
                Ok([(
                    identity.address,
                    ParsedIdentity {
    ...
    fn sign(&mut self, request: &[u8], parsed: &ParsedSignRequest<'_>) -> io::Result<Vec<u8>> {
        match &self.spec {
            ProviderSpec::ManagedPrivateKey { secret_id, .. } => {
                let encoded = self.managed_keys.read_private_key(secret_id)?;
                let signature = floria_ssh::sign(&encoded, parsed.data, parsed.flags)
                    .map_err(|error| invalid(error.to_string()))?;
...
impl SessionBindings {
    fn bind(&mut self, request: &[u8]) -> io::Result<()> {
        ...
        floria_ssh::verify_public_signature(host_key, session_id, signature)
            .map_err(|error| invalid(error.to_string()))?;
...
mod tests {
    ...
    fn runtime_socket_path_fits_unix_socket_address_for_catalog_surface_ids() {
        ...
        let path = floria_ssh::agent_runtime_socket_path(runtime_dir, surface_id);
    ...
    fn managed_snapshot(
        project: &Path,
        identity: &floria_ssh::PublicIdentity,
    ) -> CatalogSnapshot {
    ...
    fn managed_private_key_advertises_and_signs_without_an_upstream_agent() {
        ...
        let imported = floria_ssh::import_private_key(source.as_bytes(), None).unwrap();
